 ...
 *
 * <p>TODO: Implement as a *.rss Renderer</p>
 */
...
public class RssServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
    ...
    final View view = findNewsView(htmlRenderer);
    List<News> rssNews = findNews(
        servletContext,
        req,
        resp,
    ...
    {
      sb.setLength(0);
      URIEncoder.encodeURI(bookRef.getPrefix(), sb);
      URIEncoder.encodeURI(pageRef.getPath().toString(), sb);
      if (!view.isDefault()) {
        sb.append("?view=");
    ...
    for (News news : rssNews) {
      ...
      {
        sb.setLength(0);
        URIEncoder.encodeURI(targetPageRef.getBookRef().getPrefix(), sb);
        URIEncoder.encodeURI(targetPageRef.getPath().toString(), sb);
      ...
      if (recaptured == null) {
        throw new ServletException("recaptured failed: pageRef = " + newsPageRef + ", newsId = " + newsId);
      }
      if (!(recaptured instanceof News)) {
        throw new ServletException("recaptured is not news: " + recaptured.getClass().getName());
      }
      ...
      {
        sb.setLength(0);
        URIEncoder.encodeURI(newsPageRef.getBookRef().getPrefix(), sb);
        URIEncoder.encodeURI(newsPageRef.getPath().toString(), sb);
        sb.append('#');
